          ...
          >
            ...
            <div className="flex-none w-full pl-1" style={{ scrollSnapAlign: "start" }}>
              <div className="flex items-center gap-2 mb-4">
                <h2 className="text-lg font-bold text-gray-800">AI健康服务</h2>
                <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium">全部</span>
              </div>
              <div className="rounded-3xl bg-white/60 backdrop-blur-sm border border-white/50 overflow-hidden">
                {services.map((service, i) => (
                  <div key={service.id}>
                    {i > 0 && <div className="mx-4 border-t border-gray-200/60" />}
                    <button
                      onClick={() => handleServiceClick(service)}
                      className="w-full flex items-center gap-4 px-5 py-4 hover:bg-white/40 transition-colors"
                    >
                      <div className="text-2xl flex-shrink-0">{service.icon}</div>
                      <span className="flex-1 text-left text-base font-medium text-gray-800">{service.title}</span>
