...
// ── 全宽日卡片，左右滑动切换周，顶部按天跳转 ──────────────
function DayCardsFull({ weeks, startDate }: { weeks: { week: number; phase: string; tasks: string[] }[]; startDate: Date }) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [activeIdx, setActiveIdx] = useState(0); // 0-27 当前所在天
  const now = Date.now();

  const days = useMemo(() => {
    const result: { dayIndex: number; date: Date; weekNum: number; phase: string; tasks: string[] }[] = [];
    const sorted = [...weeks].sort((a, b) => a.week - b.week);
    for (const wp of sorted) {
      const ws = new Date(startDate);
      ws.setDate(ws.getDate() + (wp.week - 1) * 7);
      for (let d = 0; d < 7; d++) {
        const date = new Date(ws);
        date.setDate(date.getDate() + d);
        result.push({ dayIndex: result.length, date, weekNum: wp.week, phase: wp.phase, tasks: wp.tasks });
      }
    }
    return result;
  }, [weeks, startDate]);

  const currentWeekStart = Math.floor(activeIdx / 7) * 7; // 0, 7, 14, 21
  const currentWeek = currentWeekStart / 7 + 1; // 1-4
  const st = PHASE_STYLES[currentWeek] || PHASE_STYLES[1];
  ...
  const goWeek = (dir: -1 | 1) => {
    const target = dir < 0 ? Math.max(0, currentWeekStart - 7) : Math.min(21, currentWeekStart + 7);
    scrollTo(target);
  };
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      {/* 顶部 — 本周7天按钮 */}
      <div className="flex items-center gap-1.5 px-4 py-2 flex-shrink-0 border-b border-white/10">
        {Array.from({ length: 7 }, (_, i) => {
          const dayIdx = currentWeekStart + i;
          const isActive = activeIdx === dayIdx;
          const day = days[dayIdx];
          return (
            <button
              key={i}
              onClick={() => scrollTo(dayIdx)}
              className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium transition-all ${
                isActive
                  ? `${st.badge} border ${st.border}`
      ...
      <div
        ...
        onScroll={(e) => {
          ...
          // 对齐到周边界
          const weekSnap = Math.round(idx / 7) * 7;
          setActiveIdx(Math.min(Math.max(weekSnap, 0), 27));
        }}
      >
        {days.map((day) => {
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`h-12 w-16 rounded-2xl flex items-center justify-center ${
                    isToday ? "bg-primary/20 border border-primary/40" :
                    isPast ? "bg-green-500/15 border border-green-500/30" :
                    "bg-white/8 border border-white/15"
                  }`}>
                    <span className="text-base font-bold">DAY{day.dayIndex + 1}</span>
                  </div>
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {day.date.getMonth() + 1}/{day.date.getDate()} · 第{day.weekNum}周
                    </div>
                  </div>
                </div>
                <div>
